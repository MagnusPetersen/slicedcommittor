...
def _gridded(pi: Profile, D_q: Profile):
    """``(centers, pi, D_q, counts)`` as arrays on the density's grid, ``D_q`` interpolated."""
    ...
    return centers, pi_v, _on_grid(D_q, centers), np.asarray(pi.counts)


def flux_profiles(pi: Profile, D_q: Profile):
    """The flux ``nu_R = D_q pi`` and the diffusion, both on the density's grid,
    exactly as :func:`rate_from_profiles` integrates them: ``(nu, D_q)``."""
    centers, pi_v, Dq, counts = _gridded(pi, D_q)
    return Profile(centers, Dq * pi_v, counts), Profile(centers, Dq, counts)
...
def rate_from_profiles(
    ...
    *,
    ...
    Returns:
        ...
        ``flatness`` (:func:`flux_flatness` over it), the two
        :class:`Profile` s on the density's grid, ``nu`` (the flux) and
        ``D_q`` (the diffusion as interpolated), the diagnostics
        ``k_AB_arithmetic``, ``k_BA_arithmetic``, ``k_AB_harmonic``,
        ``k_BA_harmonic``, and the reduction's own keys: ``plateau_ok``
        (plateau), ``q_star`` (local), ``mfpt_AB``/``mfpt_BA`` (harmonic).
    """
    if reduction not in REDUCTIONS:
        raise ValueError(f"unknown reduction={reduction!r}; use one of {REDUCTIONS}")
    _own_parameters(reduction, band, q_star, window)
    rho_A, rho_B = float(rho_A), float(rho_B)

    centers, pi_v, Dq, counts = _gridded(pi, D_q)
    nu = Dq * pi_v
    nu_profile = Profile(centers, nu, counts)
    h = float(centers[1] - centers[0])
...
def committor_rate(
    ...
    * ``trajectory=`` with ``dt=`` and ``lag=`` (and ``window_ids=`` for
      umbrella data, ``run_ids=`` for replicates): the Kramers-Moyal estimate
      on the committor coordinate itself, :func:`diffusion_profile` of
      ``q(trajectory)``. This needs a diffusive regime on the committor at
      that lag; check with :func:`lag_scan`.
    ...
    if D_q is not None and any(v is not None for v in (dt, lag, window_ids, run_ids)):
        raise ValueError(
            "dt=, lag=, window_ids= and run_ids= belong to the trajectory route, not to D_q="
        )
    ...
    qx = np.asarray(committor(samples), dtype=np.float64)
    pi = density(qx, sample_weights=sample_weights, n_bins=n_bins)
    if D_q is None:
        qt = np.asarray(committor(trajectory), dtype=np.float64)
        D_q = diffusion_profile(
            qt, dt=dt, lag=lag, window_ids=window_ids, run_ids=run_ids, n_bins=n_bins
        )
