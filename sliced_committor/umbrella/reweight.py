...
PMF. MBAR is exact but materialises a ``(K, N)`` reduced-potential matrix,
which a 384-window 2D set with half a million frames cannot afford. WHAM is
binned and iterative, with cost independent of ``N`` once histogrammed, so it
...
# infeasible (K=384, N~5e5 -> ~1.6 GB just for u_kn). Auto picks WHAM.
...
def mbar_weights(
    ...
    try:
        ...
        raise ImportError(
            ...
            "'pip install sliced-committor[umbrella]' or pass method='wham'."
        ) from exc
