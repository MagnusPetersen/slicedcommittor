"""Rate-unit normalisation for cross-method / reference comparison.

Estimated rates come out in inverse dataset-time units (``1/dt`` where ``dt`` is
in the system ``time_unit``, e.g. ``ps`` for MD). Reference rates are quoted in
whatever the source used (``1/s``, ``1/us``, ...). To plot them on one axis we
normalise everything to ``1/s`` for MD systems; reduced-unit toys are left as-is
(their references share the reduced time unit).
"""
