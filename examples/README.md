...
Three self-contained walkthroughs of the public API on the rotated Wolfe-Quapp
...
when calling the library on a real problem. They share the potential, sampler,
...
| [03_sweep_wolfe_quapp.py](03_sweep_wolfe_quapp.py) | The label-free model-selection workflow: `fit_committor(..., return_details=True)` exposing the fit's variational objective `CommittorFit.dirichlet_energy` (`𝓓[q̂] = <D|∇q̂|²>`, lower = closer to the true committor), and `sweep_committor` fitting a Cartesian grid of settings and keeping the best by that objective (`res.summary()` table + `res.best_committor`). No PDE baseline and no ground truth: selection is driven purely by the variational principle. | ~ 1-2 min |
